...
RTSP/RTP送信パケットに設定するDSCP値を指定します。

初期値はEFの`46`です。`0`にするとBest Effortになります。
...
POST開始を秒単位で遅らせたい場合は「POST遅延秒」を設定します。POST送信をフルスピードにしたくない場合は「POST送信秒」を設定します。定常録画は通常1分セグメントなので、`30`を指定するとffmpegの読み込み速度を約2倍に制限し、POST全体が約30秒になるように送信します。`0`は従来通り制限なしです。
...
用途に合わせてフレームレート、ビットレート、JPEG記録、定常録画POSTの送信ペース、RTSPの一部設定をまとめて変更します。

通常、RTSP/POST、低負荷、高画質を選択できます。プロファイルを適用してから設定ボタンを押すと、通常の設定変更と同じ経路で反映されます。
...
WebUIから現在時刻、バージョン、負荷、メモリ、ストレージ、待受ポート、主要プロセス、主要設定、RTSP/録画POSTログの末尾を確認できます。
...
このforkのtoolバージョンは、fork元のベースバージョンにカスタムリビジョンを付けた `2.5.19-moogle.4` のような形式で表記します。`2.5.19` がベース、`moogle.4` がこのfork側のリビジョンです。
